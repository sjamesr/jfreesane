...
public class SaneEnums {
	...
	/**
	 * Returns a set of {@code T} obtained by treating {@code wireValue} as a
	 * bit vector whose bits represent the wire values of the enum constants of
	 * the given {@code enumType}.
	 */
	public static <T extends Enum<T> & SaneEnum> Set<T> enumSet(
			Class<T> enumType, int wireValue) {
	...
	/**
	 * Returns the result of bitwise-ORing the wire values of the given
	 * {@code SaneEnum} set. This method does not check to make sure the result
	 * is sensible: the caller must ensure that the set contains members whose
	 * wire values can be ORed together in a logically correct fashion.
	 */
